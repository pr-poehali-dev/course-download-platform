'''
Business: Извлечение PNG превью из ZIP/RAR архивов работ и загрузка в S3
Args: event с POST body {batch_size: 10, offset: 0} для пакетной обработки
        ...
        for work_id, title, download_url in works:
            try:
                ...
                if file_key.lower().endswith('.rar'):
                    skipped_rar += 1
                    errors.append(f"Work {work_id}: RAR format not supported (needs manual conversion to ZIP)")
                    continue
